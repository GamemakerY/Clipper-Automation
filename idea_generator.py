...
def get_transcript(video_ID, start_time=0, end_time=120): #start_time and end_time don't work yet
    ...
    dir_create_raw = Path(f"data/raw")
    dir_create_final = Path(f"data/processed")
    dir_create_raw.mkdir(exist_ok=True, parents=True)
    dir_create_final.mkdir(exist_ok=True, parents=True)
...
def generate_video_ideas(video_ID, number=10, special_prompt = ""): 

    output_file = f'data/raw/{video_ID}_ideas.json'
    ...
    print("Successfully generated video ideas!")
